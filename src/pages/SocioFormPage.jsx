...
const SocioFormPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await socioService.crearSocioCompleto(socioData);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => handleInputChange('usuario', 'password', e.target.value)}
                className="w-full bg-white/10 border border-white/20 rounded-lg px-4 py-3 text-white placeholder-emerald-200/50 focus:outline-none focus:ring-2 focus:ring-emerald-400"
                placeholder="Contraseña segura"
              />
              {errors['usuario.password'] && (
                <p className="mt-1 text-sm text-red-400">{errors['usuario.password']}</p>
              )}
            </div>
          </div>
        </div>
